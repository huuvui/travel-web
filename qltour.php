...
<body>
    ...
    <main>
        <div class="container">
        <h5 style="text-align: center; margin-top: 0; color: #000; font-size: 20px;"><b>DANH SÁCH TOUR CHỜ XÁC NHẬN</b></h5>
            <?php
            ...
            $sql = "SELECT bill.id AS idbill, cart.id ,bill.username, bill.name AS customer_name, cart.ten AS tour_name, cart.soluong AS quantity, cart.cost AS unit_price, bill.dia_chi AS customer_address, bill.phone AS customer_phone, bill.email AS customer_email, bill.total AS total_price, bill.time AS order_time, cart.ngaybd, bill.xacnhan
                    FROM bill
                    INNER JOIN cart ON bill.id = cart.idbill
                    WHERE bill.xacnhan = 'Đang chờ xác nhận!'
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    echo '<tr>
                            ...
                            <td>' . $row['xacnhan'] . '</td>
                            <td><a href="xacnhan.php?id=' . $row['idbill'] . '">Xác nhận</a>
                            <td><a href="huyxacnhan.php?id=' . $row['idbill'] . '">Hủy</a>
                ...
                echo '<p>Hiện tại hệ thống chưa có tour chờ xác nhận!</p>';
            }
